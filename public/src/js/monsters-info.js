...
function fetchMonster() {
  ...
  let condition_immunities = document.querySelector(
    ".monster-condition-inmunities"
  );
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      armor_class.textContent =
        data.results[0].armor_class + " (" + data.results[0].armor_desc + ")";
      hit_points.textContent =
        data.results[0].hit_points + " (" + data.results[0].hit_dice + ")";
      ...
      data.results[0].strength_save == null
        ? (save_str.innerText = "-")
        : (save_str.innerText = "+" + data.results[0].strength_save);
      data.results[0].dexterity_save == null
        ? (save_des.innerText = "-")
        : (save_des.innerText = "+" + data.results[0].dexterity_save);
      data.results[0].constitution_save == null
        ? (save_con.innerText = "-")
        : (save_con.innerText = "+" + data.results[0].constitution_save);
      data.results[0].intelligence_save == null
        ? (save_int.innerText = "-")
        : (save_int.innerText = "+" + data.results[0].intelligence_save);
      data.results[0].wisdom_save == null
        ? (save_wis.innerText = "-")
        : (save_wis.innerText = "+" + data.results[0].wisdom_save);
      data.results[0].charisma_save == null
        ? (save_cha.innerText = "-")
        : (save_cha.innerText = "+" + data.results[0].charisma_save);

      // Lista de las habilidades del monstruo
      if (Object.entries(data.results[0].skills).length !== 0) {
        for (
          let i = 0;
          i < Object.entries(data.results[0].skills).length;
          i++
        ) {
      ...
      data.results[0].condition_immunities == ""
        ...
        : (condition_immunities.innerText =
            data.results[0].condition_immunities);
      data.results[0].damage_resistances == ""
        ? (damage_resistances.innerText = "-")
        : (damage_resistances.innerText = data.results[0].damage_resistances);
      data.results[0].damage_immunities == ""
        ? (damage_immunities.innerText = "-")
        : (damage_immunities.innerText = data.results[0].damage_immunities);
      data.results[0].damage_vulnerabilities == ""
        ? (damage_vulnerabilities.innerText = "-")
        : (damage_vulnerabilities.innerText =
            data.results[0].damage_vulnerabilities);
      ...
      if (data.results[0].special_abilities !== "") {
        ...
        let specialAbilitiesContainer = document.querySelector(
      ...
      if (data.results[0].legendary_actions !== "") {
        ...
        let legendaryActionsContainer = document.querySelector(
          ".legendary-actiones-container"
        );
